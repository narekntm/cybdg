export class UserManagmentPage {
    ...
    static userTableDelete = (): Cypress.Chainable<JQuery<HTMLElement>> =>  cy.get('#user-table tbody tr').first().find('td').find('.btn-danger.delete-btn')
    static userTableEdit = (): Cypress.Chainable<JQuery<HTMLElement>> => cy.get('#user-table tbody tr').first().find('td').find('.btn-secondary.edit-btn')
    static userTableDeactivate = () :Cypress.Chainable<JQuery<HTMLElement>> => cy.get('#user-table tbody tr').eq(1).find('td').find('.btn-primary.status-btn')
    
    static userDeleteError = () :Cypress.Chainable<JQuery<HTMLElement>> => cy.get('#admin-delete-error')

    static userTableActivate = () :Cypress.Chainable<JQuery<HTMLElement>> => cy.get('#user-table tbody tr').eq(1).find('td').find('.btn-primary.status-btn')
    ...
    static usereModalCansel: Cypress.Chainable<JQuery<HTMLElement>> = cy.get('#cancel-delete')
