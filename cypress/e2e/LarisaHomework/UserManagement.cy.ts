describe("User Management Suite", () => {
  ...
  it("Login as Admin", () => {
    ...
    cy.get("input#admin-password").should("be.visible").and("be.enabled").clear().type("admin123").should("have.value", "admin123");
  ...
  it("User Management Test", () => {
    ...
    cy.get("input#name").should("be.visible").and("be.enabled").clear().type("LarisaYeremyan").should("have.value", "LarisaYeremyan");
    ...
    cy.get("select#role").should("be.visible").and("be.enabled").select("Editor").find("option").should("have.length", 4);
    ...
    cy.get("input#age").should("be.visible").and("be.enabled").clear().type("55").should("have.value", "55");
    ...
    cy.get('input[value="Male"]').parents("label").parent().siblings("label").should("have.text", "Gender");
    ...
    cy.get('input[value="Newsletter"]').parents("label").parent().siblings("label").should("have.text", "Subscribe to");
    ...
    cy.get('form#user-form button.btn-primary[type="submit"]')
      ...
      .then(() => {
        if (!userNameIsvalid) {
          cy.get('div#form-errors ul li:contains("Name must be 1–20 letters only (no spaces or symbols).")').should("be.visible");
        }

        if (!userRoleIsValid) {
          cy.get('div#form-errors ul li:contains("Role is required.")').should("be.visible");
        }

        if (!userAgeIsValid) {
          cy.get('div#form-errors ul li:contains("Age must be between 1 and 99.")').should("be.visible");
        }

        if (!userEmailIsValid) {
          cy.get('div#form-errors ul li:contains("Valid email is required.")').should("be.visible");
        }

        if (!userGenderIsValid) {
          cy.get('div#form-errors ul li:contains("Gender selection is required.")').should("be.visible");
        }

        if (userNameIsvalid && userRoleIsValid && userAgeIsValid && userEmailIsValid && userGenderIsValid)
          cy.get("table#user-table tbody tr").its("length").should("be.gt", rowCount);
      });
  });
  ...
  it("User Table", () => {
    ...
    cy.get("table#user-table thead tr")
      .first()
      .find("th")
      .each(($el, index) => {
