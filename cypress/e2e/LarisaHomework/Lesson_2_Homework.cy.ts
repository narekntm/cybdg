describe("Iterating all controls on page", () => {
  ...
  it("Get all Checkbox", () => {
    cy.get('input[type="checkbox"]:visible:enabled').check().should("be.checked");
  });

  it("Get all Radio", () => {
    cy.get('input[type="radio"]:visible:enabled').check().should("be.checked");
  });

  it("Get all Paragraph", () => {
    cy.get("p").invoke("css", "color", "green");
  });
