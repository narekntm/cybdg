...
describe("User Management Suite", () => {
	...
	it("Login as Admin, Positive case", () => {
		...
		UserManagementPage.adminEmailInput().should("be.visible")
			.and("be.enabled")
			.clear();
		...
		UserManagementPage.adminPasswordInput().should("be.visible")
			.and("be.enabled")
			.clear();
	...
	it("User Management Test, Positive case", () => {
		...
		UserManagementPage.firstNameInput().should("be.visible")
			.and("be.enabled")
			.clear();
		...
		UserManagementPage.roleSelect().should("be.visible")
			.and("be.enabled");
		...
		UserManagementPage.emailInput().should("be.visible")
			.and("be.enabled")
			.clear();
	...
	it("User Table", () => {
		...
		UserManagementPage.userTableHeader()
			.each(($el: JQuery<HTMLElement>, index: number) => {
						...
						.then(() => {
							...
							cy.wrap(randomRow)
								...
								.then(() => {
									...
									user.Subscription.forEach((value: string, index: number) => {
	...
	it("User table admin row delete", () => {
		...
		UserManagementPage.userTableHeader().then(
			($headers: JQuery<HTMLElement>) => {
				let roleColIndex = tableHeaders.indexOf("Role");
						...
						.then(($cell) => {
							if ($cell.text().trim() === roleToFind) {
								cy.wrap($row)
									...
									.click()
									.then(() => {
										UserManagementPage.adminError()
											.should("have.text", "Admin login required to delete Admin-level users.");
									});
							}
						});
				});
						...
						.then(($cell) => {
							if ($cell.text().trim() != roleToFind) {
								cy.wrap($row)
									...
									.click()
									.then(() => {
										UserManagementPage.deleteModalTitle().contains(
											"Are you sure you want to delete this user?"
										);
										UserManagementPage.deleteModalCancelBtn().should(
											"be.visible"
										);
