...
describe("User Management API Testing", () => {
  ...
  describe("Add new user", () => {
    ...
    it("Add a user with empty fields", () => {
      const invalidEmailUser: UserData = UserManagementGenerator.userEmptyDetails;
      UserManagementBuilders.AddUser(invalidEmailUser).then((response) => {
        expect(response.status).to.eq(400);
    ...
    it("Add a user with invalid age", () => {
      const invalidEmailUser: UserData = UserManagementGenerator.userNegativeAge;
      UserManagementBuilders.AddUser(invalidEmailUser).then((response) => {
        expect(response.status).to.eq(400);
    ...
    it("Add a user with invalid name", () => {
      const invalidEmailUser: UserData = UserManagementGenerator.userNegativeName;
      UserManagementBuilders.AddUser(invalidEmailUser).then((response) => {
        expect(response.status).to.eq(400);
    ...
    it("Add a user with empty gender", () => {
      const invalidEmailUser: UserData = {...UserManagementGenerator.userPositiveCase, gender: null};
      UserManagementBuilders.AddUser(invalidEmailUser).then((response) => {
        expect(response.status).to.eq(400);
    ...
    it("Edit a user", () => {
      const validUser: UserData = UserManagementGenerator.userPositiveCase;
      const editedUser: UserData = UserManagementGenerator.randomUser;
