  ...
  context('User Management Editing/Deleting Suite', () => {
    it("User table first row edit", () => {
      UserManagementPage.userTableRowTds(0).then((cells: JQuery<HTMLElement>) => {
        ...
        UserManagementPage.userTableRowEditButton(0).should("have.text", ActionButtons.Edit).should("be.visible").click();
        ...
        UserManagementPage.userTableRowTds(0).eq(Columns.Name).should('have.text', 'NewName');
      });
    });
