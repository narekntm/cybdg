export class QuizViewPage {
  ...
  static genderRadio = (value: string) => cy.get(`input[name="q2"][value="${value}"]`);

  static technologyCheckbox = (tech: string) => cy.get(`input[name="q3"][value="${tech}"]`);

  static countrySelect = () => cy.get('select[name="q4"]');

  static selectCountry = (countryValue: string) => QuizViewPage.countrySelect().select(countryValue);
