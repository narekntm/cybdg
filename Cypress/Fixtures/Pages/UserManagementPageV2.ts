...
function getInfoValue(labelText: string) {
  return cy.contains(".info-row label", labelText).parent(".info-row").find(".value");
}

export class UserManagementPageV2 {
  // main
  static mainWindow = () => cy.get(".app-wrapper");
  ...
  static newUserModal = () => cy.get("#user-form-modal .modal-content");
  ...
  static userRoleText = () => getInfoValue("Role:");
  ...
  static userTableRoleColumnTd = (row: number) =>
    cy.get("table#user-table tbody tr").eq(row).find("td").eq(UserManagementModels.Columns.Role);
  ...
  static userTableRowAdminDeleteButton = () =>
    UserManagementPageV2.userTableRows().contains(UserManagementModels.UserRole.Admin).eq(0).parent().find("td").find("button.delete-btn");
